...
hexctf_admin = json.load(open("./conf/keys_conf.json"))["0xCTFadmin"]
...
def login(name, password):
    postUrl = "http://0xctf.com/login"
    html = session.get(postUrl, headers=header)
    nonce = re.findall(
        r"name=\"nonce\" value=\"[0-9a-f]*\"", html.text)[0][19:].replace("\"", "")
    print(nonce)
    postData = {
        "name": name,
        "password": password,
...
def download_url(num):
    html = session.get(
        "http://0xctf.com/admin/submissions/correct?page={}".format(num), headers=header)
