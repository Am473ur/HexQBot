...
def msg_img_flider(rev):
    msg=rev["raw_message"].replace("[CQ:at,qq=2821876761]","").replace(" ", "")#私聊还@Hex？发空格？爪巴~直接过滤~
    ...
    try:
        ...
        img = base64.b64encode(html.content) # "http:"开头的图片链接需要get下来然后转base64才能给百度AI识别
    except:
        #print('获取该图片失败')
...
def talk_to_user(rev,talk_data):
    ...
    if is_code[0]==True:
        return is_code[1]
    #------------------------------------------------------------------------匹配对话
    return s_match(msg,talk_data)[1]
